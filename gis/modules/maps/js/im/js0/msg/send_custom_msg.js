...
//发送自定义消息
function sendCustomMsg(index) {
    if(index==1){
        //邀请加入房间状态
        var data = "{\"userAction\": \"15\",\"subcmd\": \"INVITE\",\"room\": \""+roomId+"\",\"isVideo\": \""+callType+"\",\"isSingle\": \"1\",\"caller\": \"ocean_iv\"}";//$("#ecm_data").val();
...
//        $('#edit_custom_msg_dialog').modal('hide');
        ...
        //挂断电话
        //邀请加入房间状态
        var data = "{\"userAction\": \"15\",\"subcmd\": \"HANGUP\"}";//$("#ecm_data").val();
        var desc = "";//$("#ecm_desc").val();
        var ext = "";//$("#ecm_ext").val();
        console.log(data);
        var msgLen = webim.Tool.getStrBytes(data);
