...
function onSendMsg() {
    //获取消息内容
    var msgContent = "{\"userAction\": \"15\",\"subcmd\": \"INVITE\",\"room\": \"1\",\"isVideo\": \"true\",\"isSingle\": \"true\",\"caller\": \"wxw_iv\"}";
//document.getElementsByClassName("msgedit")[0].value;
...
function handleMsgSend(msgContent){
    if (!selSess) {
        var selSess = new webim.Session(selType, "wxw_iv", "wxw_iv", friendHeadUrl, Math.round(new Date().getTime() / 1000));
    }
    var isSend = true;//是否为自己发送
    var seq = -1;//消息序列，-1表示sdk自动生成，用于去重
    var random = 1;//Math.round(Math.random() * 4294967296);//消息随机数，用于去重
    var msgTime = Math.round(new Date().getTime() / 1000);//消息时间戳
    var subType = webim.MSG_ELEMENT_TYPE.CUSTOM;//消息子类型
    if (selType == webim.SESSION_TYPE.C2C) {
