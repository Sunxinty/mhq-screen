...
var g_getUserPic; //刷新进入房间用户视频  定时器
...
function OnInit() {
    consoleLog("OnInit");
    sdk = new ILiveSDK(1400028490, 8244, "iLiveSDKCom");
...
function OnBtnLogin() {
    g_id = loginInfo.identifier;
    g_token=loginInfo.userSig;
    sdk.login(loginInfo.identifier, loginInfo.userSig, onLoginSuc, onLoginErr);
}
...
function OnBtnLogout(cb) {
    ...
    //     function(rspJson) {
            sdk.logout(function() {
                ...
                toastr.error("错误码:" + errMsg.code + " 错误信息:" + errMsg.desc,"退出登陆失败！");
            });
    //     }
    // );
    },function(errMsg){
        toastr.error("错误码:" + errMsg.code + " 错误信息:" + errMsg.desc,"退出房间失败！");
    })
}
...
function OnBtnOpenMic() {
    ...
    if (nRet.code != 0) {
        toastr.warning("获取麦克风列表出错; 错误码:" + nRet.code);
        return;
    }
...
function dealCustomMessage(user, msg) {
    ...
    if(msg.subcmd=='ACCEPT'&&msg.room==roomId){
        if(callType==1){
            OnBtnOpenCamera();
        ...
        OnBtnOpenPlayer();
        var newTimeer=0;
        $('.newTimeer').text('00:00:00');
        newTimeerInterval=setInterval(function () {
            newTimeer++;
            $('.newTimeer').text(formatSeconds(newTimeer));
        },1000);
        console.log('对方接收了通话邀请');
        toastr.success('对方接收了通话邀请');
    ...
    }else if(msg.subcmd=='HANGUP'&&msg.room==roomId){
        console.log('对方挂断了通话');
        toastr.success('对方挂断了通话');
        ...
        console.log('对方拒绝了通话邀请');
        toastr.success('对方拒绝了通话邀请');
        OnBtnLogout(function () {
            $('#callingVideoState').show();
            //$('#callingAudioState').show();
            $('#sucessAudioTongxun').show();
    ...
    else if(msg.subcmd=='CLOSE_CAMERA'){
        OnBtnCloseCamera();
        $('#sucessAudioTongxun').show();
    }
