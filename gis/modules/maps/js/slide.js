...
var pageCount = 6,nCount = 2;
function getItems(totalCount){
	var ulMap = [$('<ul class="row item active"></ul>')];
	if( totalCount > pageCount){
		var ulMapLen = Math.ceil(totalCount / pageCount);
		for(var i = 1;i<ulMapLen;i++){
			ulMap.push($('<ul class="row item"></ul>'));
		}
	}
	return ulMap;
}

function defineModal(modal,$_modal){
	if(modal){
		modal.modal('show');
	}else{
		modal = $_modal.modal({
